...
from payments.models import Transaction,SellerAccountBalance
...
def Seller_Dashboard(request, username):
    ...
    for order in orders:
        ...
        if transaction:
            ...
            overview_total_counts[overview] = overview_total_counts.get(overview, 0) + 1

        if order.status == "completed" and transaction:
            overview = transaction.overview
            amount_earned = transaction.amount

            overview_totals[overview] = overview_totals.get(overview, 0) + amount_earned
